...
#a) broj primjera za svaku vrstu pingvina na skupu za ucenje i skupu za testiranje pomocu funkcije unique

ucenje, counts = np.unique(y_train, return_counts=True)
test, counts1 = np.unique(y_test, return_counts=True)

plt.bar(ucenje-0.2, counts, width=0.2)
plt.bar(test, counts1, width=0.2)
plt.show()
...
#c) pronalazak atributa izgradjenog modela

print(LogRegression_model.intercept_)
print(LogRegression_model.coef_.T)


#d) poziv funkcije plot_decision_region

plot_decision_regions(X_train, y_train, LogRegression_model)
plt.show()


#e) klasifikacija skupa podataka, matrica zabune na testnim podacima
...
#f) dodavanje jos ulaznih velicina i prikaz rezultata
